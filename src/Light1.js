...
import lightbulb from "./light_bulb.jpg"
import lightbulboff from "./light_bulb_off.jpg"
...
export default function MediaCard() {
  ...
  if (!checked){
    ...
    fetch("http://127.0.0.1:5000/light1_automateon", {
      method: 'POST',
    ...
    fetch("http://127.0.0.1:5000/light1_automateoff", {
      method: 'POST',
    ...
    const lighton=()=>{
        setLit1(true)
        fetch("http://127.0.0.1:5000/light1_on", {
  method: "POST", // or 'PUT'
  headers: {
    "Content-Type": "application/json",
  },
  body: {"content":"light1_on"},
})
  .then((response) => response.json())
  .then((data) => {
  ...
  .catch((error) => {
    ...
    const lightoff=()=>{
        setLit1(false)
        fetch("http://127.0.0.1:5000/light1_off", {
          method: "POST", // or 'PUT'
          headers: {
            "Content-Type": "application/json",
          },
          body: {"content":"light1_off"},
        })
          .then((response) => response.json())
          .then((data) => {
            console.log("Success:", data);
